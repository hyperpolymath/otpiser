...
// Replace otpiser, otpiser, {{DEPS}} with actuals
...
| `$PREFIX/share/otpiser/`
| Data files, assets

| `$PREFIX/share/doc/otpiser/`
| Documentation
...
Default config location: `$XDG_CONFIG_HOME/otpiser/config.toml`

Fallback: `$HOME/.config/otpiser/config.toml`
...
Or via OPSM: `opsm update otpiser`
...
[source,bash]
----
just install --prefix=/opt/otpiser-instance1 --config=/etc/otpiser/instance1.toml
just install --prefix=/opt/otpiser-instance2 --config=/etc/otpiser/instance2.toml
----
